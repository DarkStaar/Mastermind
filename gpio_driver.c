...
/* Array for storing symbol ID. */
int user_symbols[4] = {0, 0, 0, 0};

/* Array for random generated symbols. */
int mastermind[4] = {0, 0, 0, 0};

/* Array for returning result of comparing user and random generated array. */
int compare_array[4] = {0, 0, 0, 0};

/* Compare array counter - global for use in blink_led function. */
int counter;

/* Temp variable for using strtol to convert binary string to integer. */
char* tmp;

/* Global variable for pressed PB0 button. */
int symbol_counter = 0;

/* Length of string. */
int len;

/* Temp variable to know how many LEDs are turned on. */
int on_count = 0;

/* 4 Kernel threads for maximum of 4 blinking LEDs. */
...
static irqreturn_t h_irq_gpio3(int irq, void *data) {
    ...
    //dodati printk i proveriti
    //strcmp koristiti
    if(strcmp(gpio_driver_buffer, "1000") == 0 || strcmp(gpio_driver_buffer, "0100") == 0 || strcmp(gpio_driver_buffer, "0010") == 0 || strcmp(gpio_driver_buffer, "0001") == 0)
    {
        if(symbol_counter < 4)
        {
            printk(KERN_INFO "Symbol counter is at: %d\n", symbol_counter);

            if(gpio_driver_buffer[0] == '1') 
            {
                user_symbols[symbol_counter] = 1;
            } 
            else if(gpio_driver_buffer[1] == '1') 
            {
                user_symbols[symbol_counter] = 2;
            } 
            else if(gpio_driver_buffer[2] == '1') 
            {
                user_symbols[symbol_counter] = 3;
            } 
            else if(gpio_driver_buffer[3] == '1') 
            {
                user_symbols[symbol_counter] = 4;
            }
        ...
        else
        {
            if(symbol_counter == 4)
            {
                printk(KERN_INFO "Symbol counter is at 4.");
                print_led();
            }
        }
    }
...
//gpio22 je za pokretanje nove igre
static irqreturn_t h_irq_gpio22(int irq, void *data) 
{
    ...
    printk(KERN_INFO "Reset has been pressed.");
...
void print_led(void)
{
        printk(KERN_INFO "Pokrenula se funkcija print led.");
        int temp;
        counter = 0;
        temp = 0;
        int i, j, k;
        for(i = 0; i < 4; i++)
        {
            if(user_symbols[i] == mastermind[i])
            {
                compare_array[temp] = 2;
                temp++;
            }else
            {
                for(j = 0; j < 4; j++)
                {
                    if(user_symbols[i] == mastermind[j])
                    {
                        compare_array[temp] = 1;
                        temp++;
                        break;
                    }
                }
        ...
        for(k = 0; k < 4; k++)
        {
            ...
            }else if(compare_array[k] == 1)
            {
                ...
                SetGpioPin(GPIO_06);
                counter++;
                    //thread0 = kthread_run(blink_led, NULL, "first thread");
                    break;

                case 1:
                SetGpioPin(GPIO_13);
                counter++;
                    //thread1 = kthread_run(blink_led, NULL, "second thread");
                    break;

                case 2:
                SetGpioPin(GPIO_19);
                counter++;
                    //thread2 = kthread_run(blink_led, NULL, "third thread");
                    break;

                case 3:
                SetGpioPin(GPIO_26);
                counter++;
                    //thread3 = kthread_run(blink_led, NULL, "fourth thread");
                    break;
...
static ssize_t gpio_driver_read(struct file *filp, char *buf, size_t len, loff_t *f_pos) {
    ...
    if (*f_pos == 0) {
        ...
        data_size = sizeof(user_symbols);

        /* Send data to user space. */
        if(symbol_counter == 4)
        {
            if (copy_to_user(buf, user_symbols, data_size) != 0) {
                return -EFAULT;
            } else {
                (*f_pos) += data_size;
                printk(KERN_INFO "Poslati korisnicki simboli.");
                return data_size;
            }
        }
    } else {
...
{
    /* Reset memory. */
    memset(mastermind, 0, BUF_LEN);

    /* Get data from user space.*/
    if (copy_from_user(mastermind, buf, len) != 0) 
    {
        return -EFAULT;
    }
    else
    {
        printk(KERN_INFO "Preuzeta kombinacija.");
        return len;
    }
}